...
function getFallbackResponse(message: string, language: string = "en"): string {
  ...
  if (isDe) {
    if (lower.includes("konto") || lower.includes("eröffnen") || lower.includes("registrier") || lower.includes("mitglied")) {
    ...
    if (lower.includes("pin") || lower.includes("code") || lower.includes("sicherheit")) {
      return "Ihre 4-stellige Transaktions-PIN sichert alle Überweisungen und Abrechnungen. Sie finden diese in Ihren Kontodetails oder können sie über das Kundenportal bzw. Ihren Berater anfordern.";
    }
  ...
  if (isFr) {
    ...
    if (lower.includes("pin") || lower.includes("code")) {
      return "Votre code PIN à 4 chiffres protège l'ensemble de vos transactions et virements. Vous pouvez le consulter en toute confidentialité dans les paramètres de votre compte ou auprès de votre gestionnaire dédié.";
    }
  ...
  if (isEs) {
    ...
    if (lower.includes("pin") || lower.includes("código")) {
      return "Su PIN de transacción de 4 dígitos protege todas sus transferencias financieras. Puede visualizarlo de manera segura en la información de su cuenta o gestionarlo con su banquero privado.";
    }
  ...
  if (lower.includes("pin") || lower.includes("transaction pin") || lower.includes("4 digit") || lower.includes("code")) {
    return "Your 4-Digit Transaction PIN is an encrypted security key required to authorize international wires, card actions, and account modifications. You can view or request an update to your transaction PIN anytime inside your Account Details portal or through your dedicated Relationship Manager.";
  }
  if (lower.includes("cot") || lower.includes("swift") || lower.includes("imf") || lower.includes("aml") || lower.includes("clearance")) {
    return "Global Elite Bank complies with Swiss Federal Banking (FINMA) and Basel III standards. High-value international cross-border transfers include multi-factor clearance validation (SWIFT, COT, IMF, and AML clearances) to protect client assets from unauthorized repatriation.";
  }
  if (lower.includes("open") || lower.includes("account") || lower.includes("apply") || lower.includes("member")) {
    return "To apply for an exclusive account with Global Elite Bank, please click the 'Apply for Membership' button in the navigation header. Our private admissions committee reviews high-net-worth and institutional applications within 24 hours with utmost discretion.";
  }
  if (lower.includes("transfer") || lower.includes("wire") || lower.includes("limit")) {
  ...
  if (lower.includes("crypto") || lower.includes("bitcoin") || lower.includes("eth") || lower.includes("sol") || lower.includes("custody") || lower.includes("vault")) {
  ...
  if (lower.includes("card") || lower.includes("visa") || lower.includes("mastercard")) {
    return "Global Elite Bank issues bespoke Metal Titanium and Virtual Black Cards with customizable spending limits, zero foreign transaction fees, and instant tokenized digital wallet provisioning.";
  }
  ...
  return "Welcome to Global Elite Bank. As your AI Private Wealth Concierge, I can assist you with numbered accounts, international SWIFT wires, clearance codes, Swiss crypto custody, and private wealth solutions. How may I assist you today?";
}
...
- Transaction PIN: Every user account is equipped with an encrypted 4-digit transaction PIN required for wire authorizations and security verifications.
...
- Security Policy: Never ask for confidential passwords, transfer PINs, or private keys. Remind users that Global Elite Bank will never ask for client credentials.`;
    ...
    const reply = response.text || getFallbackResponse(message, language);
    return res.json({
      reply,
      model: "gemini-2.5-flash",
      source: "gemini-api",
